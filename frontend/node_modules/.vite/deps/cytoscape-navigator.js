...
//#region node_modules/cytoscape-navigator/cytoscape-navigator.js
var require_cytoscape_navigator = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function() {
		"use strict";
		var defaults = {
			container: false,
		...
		Navigator.prototype = {
			...
			destroy: function() {
				...
				if (this.options.container && !this.options.removeCustomContainer) this.$panel.innerHTML = "";
				else this.$panel.parentElement.removeChild(this.$panel);
			},
			_initPanel: function() {
				var options = this.options;
				if (options.container && typeof options.container === "string" && options.container.length > 0) {
					if (options.container.indexOf("#") !== -1) this.$panel = document.getElementById(options.container.replace("#", ""));
					else this.$panel = document.getElementsByClassName(options.container.replace(".", ""))[0];
				} else {
			...
			_checkThumbnailSizesAndUpdate: function() {
				...
				if (_zoom != this.thumbnailZoom || _pan_x != this.thumbnailPan.x || _pan_y != this.thumbnailPan.y) {
					this._setupThumbnail();
					this._setupView();
				} else this._updateThumbnailImage();
			},
			_initView: function() {
			...
			_initEventsHandling: function() {
				...
				var overlayListener = function(ev) {
					...
					if (ev.type == "mousedown" || ev.type == "touchstart") that._eventMoveStart(ev);
					else if (ev.type == "mousewheel" || ev.type == "DOMMouseScroll") that._eventZoom(ev);
					return false;
				};
				var globalListener = function(ev) {
					...
					if (ev.type == "mousemove" || ev.type == "touchmove") that._eventMove(ev);
					else if (ev.type == "mouseup" || ev.type == "touchend") that._eventMoveEnd(ev);
					return false;
				};
				for (var i = 0; i < eventsLocal.length; i++) this.$overlay.addEventListener(eventsLocal[i], overlayListener, false);
				for (var i = 0; i < eventsGlobal.length; i++) window.addEventListener(eventsGlobal[i], globalListener, false);
				this._removeEventsHandling = function() {
					for (var i = 0; i < eventsLocal.length; i++) this.$overlay.removeEventListener(eventsLocal[i], overlayListener);
					for (var i = 0; i < eventsGlobal.length; i++) window.removeEventListener(eventsGlobal[i], globalListener);
				};
			},
			...
			_eventMove: function(ev) {
				...
				if (this.options.viewLiveFramerate !== false) {
					if (this.options.viewLiveFramerate == 0) this._moveCy();
					else if (!this.overlayTimeout) this.overlayTimeout = setTimeout(function() {
						that._moveCy();
			...
			_checkMousePosition: function(ev) {
				if (ev.offsetX > this.viewX && ev.offsetX < this.viewX + this.viewBorderHorizontal + this.viewW && ev.offsetY > this.viewY && ev.offsetY < this.viewY + this.viewBorderVertical + this.viewH) this.$panel.classList.add("mouseover-view");
				else this.$panel.classList.remove("mouseover-view");
			},
			_eventMoveEnd: function(ev) {
				...
				if (!this.overlayInMovement) return;
				this._eventMove(ev);
				if (this.options.viewLiveFramerate === false) this._moveCy();
				this.overlayInMovement = false;
			},
			_eventZoom: function(ev) {
				...
				if (this.cy.zoomingEnabled()) this._zoomCy(zoomRate, mousePosition);
			},
			_updateThumbnailImage: function() {
				var that = this;
				if (this._thumbnailUpdating) return;
				...
				var render = function() {
					...
					if (png.indexOf("image/png") < 0) img.removeAttribute("src");
					else img.setAttribute("src", png);
					var translate = {
						x: (w - zoom * bb.w) / 2,
						y: (h - zoom * bb.h) / 2
					};
		...
		if (typeof module !== "undefined" && module.exports) module.exports = function(cytoscape) {
			register(cytoscape);
		};
		else if (typeof define !== "undefined" && define.amd) define("cytoscape-navigator", function() {
			return register;
		});
